...
| GET `/dataobjs`       | All optional: `path` (filter by directory, recursive), `type` (repeatable, OR), `tags` (repeatable, AND), `sort` (`modified_at`\|`date`\|`title`\|`id`, default `modified_at`), `order` (`asc`\|`desc`, default `desc`), `page` (default 1), `per_page` (1-100, default: all on one page) | Returns a filtered, sorted and paginated page of dataobjs as `{"data": [...], "pagination": {...}}`. With no parameters every dataobj is returned on a single page. The `pagination` object holds `page`, `per_page`, `total_items`, `total_pages`, `has_next`, `has_prev`, `sort` and `order`. Returns `400` on invalid parameters. |
